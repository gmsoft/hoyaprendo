...
User = function(connection, response) {
	function updateLastLogin(pUser_id) {
		var query = 'UPDATE user SET lastlogin = NOW() WHERE user_id = ?;';
		connection.query(query, [pUser_id], function(error, rows) {
			if (!error) {
				switch(rows.changedRows) {
					case 1:
						console.log('User.updateLastLogin = OK');
						break;
					case 0:
						console.error('User.updateLastLogin = Not updated');
						break;
					default:
						console.error('User.updateLastLogin = Duplicity error');
						break;
				}
			} else {
				console.error('User.updateLastLogin = ' + error.stack);
			}
		});
	}
